...
Figma: `내 정보 관리` 섹션, node `13460:62915` (파일 `0IkM9YTgD0h4KdlUpclGiB`). 데스크톱 기본/입력완료 프레임(`13460:62916`, `13460:62988`)은 `get_design_context`로 실제 레퍼런스 코드 확인, 태블릿/모바일 및 토스트 프레임(`13460:63061`, `13460:63199`, `13460:63479`, `13460:63337`, `13460:63408`)은 `get_metadata`로 좌표/크기 확인 완료.
...
`get_metadata`로 데스크톱(1920)/태블릿(744)/모바일(375) 프레임을 모두 확인. 이 프로젝트는 Tailwind 기본 브레이크포인트(`md`=768, `lg`=1024)를 이미 base/md/lg 3단으로 쓰고 있음([Button.tsx](../../../components/common/Button.tsx)의 `text-sm md:text-base lg:text-lg` 패턴) — 그대로 따른다: 기본값=모바일, `md:`=태블릿, `lg:`=데스크톱.

- **사이드메뉴는 신경 쓸 필요 없음** — `(with-header)/layout.tsx`가 렌더링하는 [SideMenu](../../../components/layout/SideMenu/SideMenu.tsx)가 이미 데스크톱(전체 사이드바 362px)/태블릿(아이콘만 60px)/모바일(상단 `gnb_mobile` 바로 전환, 사이드바 없음) 반응형을 자체적으로 구현하고 있음. `/mypage`는 그냥 `(with-header)` 그룹 안에 있기만 하면 됨
- **카드 폭이 고정이 아님**: 데스크톱만 폭 560px 고정(중앙 콘텐츠 영역), 태블릿/모바일은 사이드바 옆 남은 영역을 꽉 채우는 유동폭(태블릿 628px, 모바일 343px selon 뷰포트) — `w-full max-w-[560px]` 같은 방식으로 구현하고 하드코딩된 560px를 모든 브레이크포인트에 쓰지 않는다
- **input/button 높이**: 모바일만 더 작음 (input 44px, 버튼 48px) / 태블릿·데스크톱은 56px로 동일. 기존 `FormInput`/`Button` 컴포넌트가 이미 이런 반응형 사이즈 variant를 갖고 있는지 구현 시 확인 필요
- **토스트 폭도 고정 688px가 아님** — 콘텐츠 영역 폭에 맞춰 스케일됨: 데스크톱 688px(콘텐츠 560px보다 넓게 좌우 여백 포함), 태블릿 628px(콘텐츠 폭과 동일), 모바일 343px(콘텐츠 폭과 거의 동일). 화면 하단 근처, 콘텐츠 영역과 수평 정렬되도록 구현 (전체 뷰포트 기준 항상 정중앙이 아님)
- 기본(빈 값) 상태의 비밀번호 placeholder 텍스트 확인됨: "현재 비밀번호를 입력해주세요" / "새 비밀번호를 입력해주세요" / "새 비밀번호를 다시 입력해주세요"
