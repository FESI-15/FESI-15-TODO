  ...
  return (
    <section className="flex w-full flex-col items-center bg-gray-50 px-4 py-11 md:gap-13.5 md:px-6 md:pt-25 md:pb-17.5 lg:px-10 lg:py-30">
      <div className="flex w-full max-w-348 flex-col items-center gap-11 md:gap-18 lg:gap-22">
        <m.div
          className={headingContainerVariants({ align: "center" })}
          initial="hidden"
          whileInView="visible"
        ...
        >
          ...
          >
            {renderAnimatedWords("쉽고 빠르게 할 일을 시작해요")}
          </m.h2>
        </m.div>
        <ul className="flex w-full flex-col gap-4 md:flex-row lg:gap-8">
          {STEPS.map(({ icon, title, description }, index) => (
            <li key={title} className="flex-1">
              <Reveal delay={index * 0.3} className="h-full">
                <m.div
                  whileHover={{
                    y: -6,
                    scale: 1.02,
                  ...
                  className="flex h-full flex-col items-center gap-6 rounded-3xl bg-white px-6 py-10 shadow-[0px_0px_60px_0px_rgba(0,0,0,0.05)] md:gap-8 md:px-8 md:py-12 lg:gap-6 lg:rounded-[40px] lg:px-12 lg:py-16"
                >
                  <m.span
                    ...
                    className="flex h-9 w-9 items-center justify-center rounded-full bg-orange-500 text-xl leading-7.5 font-bold text-white shadow-[0px_4px_8px_0px_rgba(255,158,89,0.2)]"
                  >
                    {index + 1}
                  </m.span>
